...
# Bot Bureau — 常驻本机的 AI 成员团队（Go + Electron）

**Bot Bureau** 是一间跑在你自己机器上的「AI 事务所」：一支常驻的 AI 成员团队，你像给成员发消息一样派活，他们独立完成多步骤任务、需要人工决策时回来找你审批、记住你的偏好、把重复的活存成定时例程，彼此还能在群聊里分工协作。
...
| 能力 | 实现 |
|---|---|
| 像给成员发消息一样 | **群聊**（直接喊名字或 @ 都能派活，谁都没点到时由群里第一位成员接手；bot 协作全程可见）+ **私聊**（一对一独立干活） |
...
| — | **空手起步**：出厂没有 bot、没有群聊；首次打开走三步引导（你是老板 → 能做什么 → 请第一位成员），可跳过 |
...
**第一次打开是空的**——没有 bot，也没有群聊。引导会带你走三步：先说清楚你是这家事务所的老板、bot 是你雇来的成员，再指一遍创建和管理的入口，最后请第一位成员（选服务商 → 登录订阅或填 key → 从拉取到的列表里选模型）。跳过也行，侧栏的 ＋ 随时能补。
...
首次启动会用同一套面板做一次引导：选一次，套给所有还没配模型的成员；之后每位成员都能单独改。
...
在这里它就是一位真正的成员——有自己的工作目录、记忆、任务看板身份，能被 @、能被派活。
