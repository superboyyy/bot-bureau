...
# Bot Bureau —— 在自己的电脑上运行的常驻 AI 成员团队（Go + Electron）
...
| 能力 | 说明 |
|---|---|
| 像团队成员一样发消息 | **群聊**中可以直接叫成员名字或 @；没有点名时由群里的第一位成员处理，其他成员只旁听上下文；**私聊**则是一对一独立工作 |
...
| 不必输入 @ | 在群聊中直接说 `scout 看一下` 等同于 `@scout`，不会误匹配更长的单词 |
...
- 新建群聊，加入几位成员，然后说：`scout 查一下今天的 AI 新闻`（不必输入 @）。
...
图标由 `assets/logo.svg` 和 `assets/icon-layers/` 生成浅色、深色两套资源。应用运行时，主进程监听系统外观变化并切换 Dock / 任务栏图标；应用未运行时，macOS 使用打包时写入的固定图标。可选的 `assets/AppIcon.icon` 会在 `afterPack` 阶段由 `actool` 编译并写入包内，缺少完整 Xcode 时会跳过这一步而不影响构建。
...
- **移动端（iOS / Android）**：提供原生客户端，在电脑或服务器上的引擎持续运行时，用手机聊天、查看任务进度并处理审批。

移动端不会运行引擎；模型调用、bash 和插件仍由常驻设备负责。原生客户端上线前，可以使用 Telegram 桥接作为手机入口。

| 平台 | 形态 | 状态 |
|---|---|---|
| macOS / Windows / Linux | Electron，内嵌引擎 | ✅ 已发布 |
| Telegram | 桥接，任何能运行 Telegram 的设备都可使用 | ✅ 已支持 |
| iOS / Android | 连接到引擎设备的原生客户端 | 计划中 |
| watchOS | 查看任务进度、接收审批推送并批准 / 拒绝 | 计划中 |

手机和手表客户端不会运行引擎；模型调用、bash 和插件仍由常驻设备负责。watchOS 客户端会保持窄范围，只处理“成员在等你”的任务列表、批准 / 拒绝和进度查看。
...
servers:
  ...
  - name: linear
    ...
    bearer_key: LINEAR_TOKEN
~~~

工具名会以 `mcp_<plugin>_<tool>` 的形式提供给订阅该插件的成员。MCP 放在引擎层而不是某个服务商 API 内，因此 Grok、DeepSeek、Ollama 等所有成员都能使用同一套插件。远程连接器可以使用静态 bearer token 或 OAuth；OAuth 流程包含资源元数据发现、授权服务器发现、动态客户端注册、PKCE 和自动刷新。
...
云端常驻虚拟机、代替用户登录真实网站、通过一次示范自动学习流程，以及原生移动端提醒和客户端暂未实现；Telegram 桥接提供了当前的手机入口。
