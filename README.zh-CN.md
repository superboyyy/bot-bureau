...
图标由 `assets/logo.svg` 和 `assets/icon-layers/` 生成浅色、深色两套资源。打包进应用的图标与 macOS 一样裁成带边距的超椭圆（圆角），Windows 的 `.ico` 与 Linux 的 PNG 不再使用直角方图。应用运行时，主进程监听系统外观变化并切换 Dock / 任务栏图标；应用未运行时，macOS 使用打包时写入的固定图标。可选的 `assets/AppIcon.icon` 会在 `afterPack` 阶段由 `actool` 编译并写入包内，缺少完整 Xcode 时会跳过这一步而不影响构建。
